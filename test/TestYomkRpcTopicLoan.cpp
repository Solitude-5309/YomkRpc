...
void check(const std::string &name, bool condition)
{
    if (condition)
    {
        YOMK_INFO_TAG("TestYomkRpcTopicLoan", "[PASS] ", name);
        g_pass++;
    }
    else
    {
        YOMK_ERROR_TAG("TestYomkRpcTopicLoan", "[FAIL] ", name);
        g_fail++;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // 测试创建节点
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "=== Test YomkRpcService::createNode ===");
    ...
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "=== Test loan fallback (non-plain MString) ===");
    ...
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "=== Test loan publish (plain MInt32) ===");
    ...
    auto onLoan = [&](const void *data)
    {
        ...
        loanReceived++;
        YOMK_INFO_TAG("TestYomkRpcTopicLoan", "[RECV] 收到借出消息: ", msg->data());
    };
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            loanPublished = false;
            YOMK_ERROR_TAG("TestYomkRpcTopicLoan", "[SEND] 借出失败: 第 ", i, " 次 loan 返回 nullptr");
            break;
        }
        ...
        {
            loanPublished = false;
            YOMK_ERROR_TAG("TestYomkRpcTopicLoan", "[SEND] 借出发布失败: ", 100 + i, " status=", static_cast<int>(resp.m_status));
            break;
        }
        YOMK_INFO_TAG("TestYomkRpcTopicLoan", "[SEND] 借出发布: ", 100 + i);
    ...
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "[RESULT] 接收=", recvCount, "/5 最后值=", lastValueCopy);
    ...
    // 测试 discard：借出后放弃归还
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "=== Test discard loan ===");
    YOMKRPC_LOAN("node0", "loan_topic", loanPtr);
    ...
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "=== Test YomkRpcService::deleteNode ===");
    ...
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "========== Test Summary ==========");
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "PASS: ", g_pass);
    YOMK_INFO_TAG("TestYomkRpcTopicLoan", "FAIL: ", g_fail);
