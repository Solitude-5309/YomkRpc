...
// 与 TestYomkRpcPub（发布端示例）配合可演示真实跨进程发布/订阅通信
int main(int argc, char *argv[])
{
    ...
    {
        YOMK_ERROR_TAG("TestYomkRpcSub", "create node failed: ", resp.m_msg);
        return 1;
    }
    ...
    auto onMessage = [&](const void *data)
    {
        ...
        received++;
        YOMK_INFO_TAG("TestYomkRpcSub", "[RECV] ", msg->data());
    };
    resp = YOMKRPC_SUB_TOPIC("sub_node", "hello_world", new YomkRpc::MStringPubSubType(), onMessage);
    if (resp.m_status != YomkResponse::eOk)
    {
        YOMK_ERROR_TAG("TestYomkRpcSub", "register sub topic failed: ", resp.m_msg);
    ...
    YOMK_INFO_TAG("TestYomkRpcSub", "subscribing hello_world, press Ctrl+C to exit");
    while (!g_stop.load())
    {
    ...
    YOMK_INFO_TAG("TestYomkRpcSub", "received total=", received.load(), " messages");
    resp = YOMKRPC_DEL_NODE("sub_node");
    if (resp.m_status != YomkResponse::eOk)
    {
        YOMK_ERROR_TAG("TestYomkRpcSub", "delete node failed: ", resp.m_msg);
        return 1;
    }
    return 0;
}
