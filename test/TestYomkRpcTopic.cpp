...
void check(const std::string &name, bool condition)
{
    if (condition)
    {
        YOMK_INFO_TAG("TestYomkRpcTopic", "[PASS] ", name);
        g_pass++;
    }
    else
    {
        YOMK_ERROR_TAG("TestYomkRpcTopic", "[FAIL] ", name);
        g_fail++;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // 测试版本查询（宏内部解包并打印）
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::getVersion ===");
    YOMKRPC_VERSION();

    // 测试创建节点
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::createNode ===");
    ...
    // 测试注册发布主题
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::registerPubTopic ===");
    ...
    // 测试注册订阅主题
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::registerSubTopic ===");
    ...
    auto onMessage = [&](const void *data)
    {
        ...
        lastMsg = msg->data();
        received++;
        YOMK_INFO_TAG("TestYomkRpcTopic", "[RECV] 收到消息: ", msg->data());
    };
    ...
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::publish ===");
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            allPublished = false;
            YOMK_ERROR_TAG("TestYomkRpcTopic", "[SEND] 发布失败: ", msg.data(), " status=", static_cast<int>(resp.m_status));
            break;
        }
        YOMK_INFO_TAG("TestYomkRpcTopic", "[SEND] 发布消息: ", msg.data());
    ...
    YOMK_INFO_TAG("TestYomkRpcTopic", "[RESULT] 接收=", recvCount, "/5 最后一条=", lastMsgCopy);
    ...
    YOMK_INFO_TAG("TestYomkRpcTopic", "=== Test YomkRpcService::deleteNode ===");
    ...
    YOMK_INFO_TAG("TestYomkRpcTopic", "========== Test Summary ==========");
    YOMK_INFO_TAG("TestYomkRpcTopic", "PASS: ", g_pass);
    YOMK_INFO_TAG("TestYomkRpcTopic", "FAIL: ", g_fail);
