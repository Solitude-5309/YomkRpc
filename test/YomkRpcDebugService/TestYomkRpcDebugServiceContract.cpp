 ...
 *
 * 范围：仅验证调试服务 8 端点在不触发任何 DDS 运行时（不创建 participant）前提下的
 *       输入校验与错误码契约；真实 DDS 生命周期（创建/删除/登记）归
 *       TestYomkRpcDebugServiceLifecycle，节点层守卫与端到端流量归 TestFastDDSDebugNode。
 * 覆盖：
 *   T1 funcInfos 内省 8 端点齐全 + 未知端点 eNo；
 *   T2 /version 正常路径（eOk + 版本串契约 + 忽略 pkg）；
 *   T3 /create_node、/topic_print、/list_topics、/topic_info、/list_nodes、/node_info 解包双守卫
 *      （nullptr / 异类包 / 改名伪造）；
 ...
 * DDS-free 保证：T5 的越界校验在锁外返回；T4/T6/T7/T8/T9 的 node_ 空检查在触达节点层前返回；
...
int main()
{
    ...
    CHECK(infos.size() == 8 && infos.count("/version") && infos.count("/create_node") &&
